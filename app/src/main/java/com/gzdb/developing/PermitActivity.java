...
public class PermitActivity extends AfinalActivity implements View.OnClickListener {
    ...
    private void saveDeveLoping() {
        ...
        if (edit_permitno.getText().toString().length() == 0) {
            ToastUtil.showToast(PermitActivity.this, "请输入正确的许可证编号");
            return;
        }
        if (edit_businesslicense_no.getText().toString().length() == 0) {
            ToastUtil.showToast(PermitActivity.this, "请输入正确的营业执照号");
            return;
        }
        if (permitImg.length() == 0) {
            ToastUtil.showToast(PermitActivity.this, "请输上传许可证");
            return;
        }
        if (businesslicenseImg.length() == 0) {
            ToastUtil.showToast(PermitActivity.this, "请输上传营业执照");
            return;
        }
        Map map = new HashMap();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_next:
                startActivity(new Intent(PermitActivity.this, NextDevelopingActivity.class));
                break;
    ...
    public Dialog createWheelViewDialog() {
        ...
        View view = View.inflate(PermitActivity.this, R.layout.showdialog,
                null);
